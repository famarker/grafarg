...
var (
	...
	buildId               string   = "0"
	serverBinary          string   = "grafarg-server"
	cliBinary             string   = "grafarg-cli"
...
func main() {
	...
	flag.BoolVar(&enterprise, "enterprise", enterprise, "Build enterprise version of Grafarg")
	...
	flag.BoolVar(&printGenVersion, "gen-version", printGenVersion, "generate Grafarg version and output (default: false)")
	flag.Parse()
	...
	for _, cmd := range flag.Args() {
		...
		case "build-srv", "build-server":
			clean()
			doBuild("grafarg-server", "./pkg/cmd/grafarg-server", buildTags)

		case "build-cli":
			clean()
			doBuild("grafarg-cli", "./pkg/cmd/grafarg-cli", buildTags)
...
func makeLatestDistCopies() {
	...
	latestMapping := map[string]string{
		"_amd64.deb":               "dist/grafarg_latest_amd64.deb",
		".x86_64.rpm":              "dist/grafarg-latest-1.x86_64.rpm",
		".linux-amd64.tar.gz":      "dist/grafarg-latest.linux-x64.tar.gz",
		".linux-amd64-musl.tar.gz": "dist/grafarg-latest.linux-x64-musl.tar.gz",
		".linux-armv7.tar.gz":      "dist/grafarg-latest.linux-armv7.tar.gz",
		".linux-armv7-musl.tar.gz": "dist/grafarg-latest.linux-armv7-musl.tar.gz",
		".linux-armv6.tar.gz":      "dist/grafarg-latest.linux-armv6.tar.gz",
		".linux-arm64.tar.gz":      "dist/grafarg-latest.linux-arm64.tar.gz",
		".linux-arm64-musl.tar.gz": "dist/grafarg-latest.linux-arm64-musl.tar.gz",
	}
...
func setup() {
	...
	args = append(args, "./pkg/cmd/grafarg-server")
	runPrint("go", args...)
}
...
func clean() {
	...
	rmr("dist")
	rmr("tmp")
	rmr(filepath.Join(build.Default.GOPATH, fmt.Sprintf("pkg/%s_%s/github.com/famarker", goos, goarch)))
}
