...
// Or from the commandline: GRAFARG_TEST_DB=mysql go test -v ./pkg/tsdb/mysql
// The tests require a MySQL db named grafarg_ds_tests and a user/password grafarg/password
...
func InitMySQLTestDB(t *testing.T) *xorm.Engine {
	...
	x, err := xorm.NewEngine(testDB.DriverName, strings.Replace(testDB.ConnStr, "/grafarg_tests",
		"/grafarg_ds_tests", 1))
	if err != nil {
