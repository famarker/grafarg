...
func TestAzureMonitorBuildQueries(t *testing.T) {
	...
	commonAzureModelProps := map[string]interface{}{
		"aggregation":      "Average",
		"resourceGroup":    "grafargstaging",
		"resourceName":     "grafarg",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			azureMonitorQuery := &AzureMonitorQuery{
				URL: "12345678-aaaa-bbbb-cccc-123456789abc/resourceGroups/grafargstaging/providers/Microsoft.Compute/virtualMachines/grafarg/providers/microsoft.insights/metrics",
		...
		},
		{
			...
			expectedFrames: data.Frames{
				data.NewFrame("",
					...
					data.NewField("Percentage CPU", nil, []*float64{
						ptr.Float64(8.26), ptr.Float64(8.7), ptr.Float64(14.82), ptr.Float64(10.07), ptr.Float64(8.52),
					}).SetConfig(&data.FieldConfig{Unit: "percent", DisplayName: "custom grafargstaging Microsoft.Compute/virtualMachines grafarg Percentage CPU"})),
			},
		},
		{
