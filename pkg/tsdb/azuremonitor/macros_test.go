...
func TestAzureLogAnalyticsMacros(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			kql:      `CounterPath in ($__escapeMulti('\\grafarg-vm\Network(eth0)\Total','\\grafarg-vm\Network(eth1)\Total'))`,
			expected: `CounterPath in (@'\\grafarg-vm\Network(eth0)\Total', @'\\grafarg-vm\Network(eth1)\Total')`,
			Err:      require.NoError,
		},
		{
			...
			kql:      `$__escapeMulti('\\grafarg-vm,\Network(eth0)\Total Bytes Received')`,
			expected: `@'\\grafarg-vm,\Network(eth0)\Total Bytes Received'`,
			Err:      require.NoError,
		},
		{
