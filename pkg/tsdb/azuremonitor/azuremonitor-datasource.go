...
func (e *AzureMonitorDatasource) createRequest(ctx context.Context, dsInfo *models.DataSource) (*http.Request, error) {
	...
	req.Header.Set("User-Agent", fmt.Sprintf("Grafarg/%s", setting.BuildVersion))
	...
	for _, series := range amr.Value[0].Timeseries {
		...
		if amr.Value[0].Unit != "Unspecified" {
			dataField.SetConfig(&data.FieldConfig{
				Unit: toGrafargUnit(amr.Value[0].Unit),
			})
		}
		if query.Alias != "" {
...
// to
//   https://github.com/famarker/grafarg/blob/master/packages/grafarg-data/src/valueFormats/categories.ts#L24
func toGrafargUnit(unit string) string {
	switch unit {
	case "BitsPerSecond":
		return "bps"
	case "Bytes":
	...
	// 1. Do not have a corresponding unit in Grafarg's current list.
