...
// Or from the commandline: GRAFARG_TEST_DB=postgres go test -v ./pkg/tsdb/postgres
// The tests require a PostgreSQL db named grafargdstest and a user/password grafargtest/grafargtest!
...
func InitPostgresTestDB(t *testing.T) *xorm.Engine {
	...
	x, err := xorm.NewEngine(testDB.DriverName, strings.Replace(testDB.ConnStr, "dbname=grafargtest",
		"dbname=grafargdstest", 1))
