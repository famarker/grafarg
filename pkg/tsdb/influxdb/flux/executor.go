	...
	if err != nil {
		...
		if dr.Error != nil {
			...
			if errors.As(dr.Error, &maxPointError) {
				text := fmt.Sprintf("A query returned too many datapoints and the results have been truncated at %d points to prevent memory issues. At the current graph size, Grafarg can only draw %d.", maxPointError.Count, query.MaxDataPoints)
