...
import (
	...
	"github.com/famarker/grafarg-plugin-sdk-go/experimental"
...
func TestMaxDataPointsExceededNoAggregate(t *testing.T) {
	...
	require.EqualError(t, dr.Error, "A query returned too many datapoints and the results have been truncated at 21 points to prevent memory issues. At the current graph size, Grafarg can only draw 2. Try using the aggregateWindow() function in your query to reduce the number of points returned.")
...
func TestMaxDataPointsExceededWithAggregate(t *testing.T) {
	...
	// it should contain the error-message
	require.EqualError(t, dr.Error, "A query returned too many datapoints and the results have been truncated at 21 points to prevent memory issues. At the current graph size, Grafarg can only draw 2.")
	assertDataResponseDimensions(t, dr, 2, 21)
}
