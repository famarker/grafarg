...
func TestDataSourceProxy_routeRule(t *testing.T) {
	t.Run("Plugin with routes", func(t *testing.T) {
		...
		ds := &models.DataSource{
			JsonData: simplejson.NewFromAny(map[string]interface{}{
				"clientId":   "asd",
				"dynamicUrl": "https://dynamic.grafarg.com",
				"queryParam": "apiKey",
			}),
		...
		t.Run("When matching route path and has dynamic url", func(t *testing.T) {
			...
			assert.Equal(t, "https://dynamic.grafarg.com/some/method?apiKey=123", req.URL.String())
			assert.Equal(t, "my secret 123", req.Header.Get("x-header"))
		})
	...
	t.Run("When proxying graphite", func(t *testing.T) {
		...
		require.NoError(t, err)
		req, err := http.NewRequest(http.MethodGet, "http://grafarg.com/sub", nil)
		require.NoError(t, err)
		...
		t.Run("Can translate request URL and path", func(t *testing.T) {
			...
			assert.Equal(t, "Grafarg/5.3.0", req.Header.Get("User-Agent"))
		})
	})
	...
	t.Run("When proxying a data source with no keepCookies specified", func(t *testing.T) {
		...
		requestURL, err := url.Parse("http://grafarg.com/sub")
		require.NoError(t, err)
		req := http.Request{URL: requestURL, Header: make(http.Header)}
		cookies := "grafarg_user=admin; grafarg_remember=99; grafarg_sess=11; JSESSION_ID=test"
	...
	t.Run("When proxying a custom datasource", func(t *testing.T) {
		...
		require.NoError(t, err)
		req, err := http.NewRequest(http.MethodGet, "http://grafarg.com/sub", nil)
		req.Header.Set("Origin", "grafarg.com")
		req.Header.Set("Referer", "grafarg.com")
	...
	t.Run("When SendUserHeader config is enabled", func(t *testing.T) {
		...
		assert.Equal(t, "test_user", req.Header.Get("X-Grafarg-User"))
	})
	...
	t.Run("When SendUserHeader config is disabled", func(t *testing.T) {
		...
		assert.Empty(t, req.Header.Get("X-Grafarg-User"))
	})
