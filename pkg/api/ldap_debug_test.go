...
import (
	...
	"github.com/famarker/grafarg/pkg/services/multildap"
...
func TestGetUserFromLDAPAPIEndpoint_OrgNotfound(t *testing.T) {
	...
	userSearchConfig = ldap.ServerConfig{
		...
		Groups: []*ldap.GroupToOrgRole{
			{
				GroupDN: "cn=admins,ou=groups,dc=grafarg,dc=org",
				OrgId:   1,
...
func TestGetUserFromLDAPAPIEndpoint(t *testing.T) {
	...
	userSearchResult = &models.ExternalUserInfo{
		...
		Email:          "john.doe@example.com",
		Login:          "johndoe",
		Groups:         []string{"cn=admins,ou=groups,dc=grafarg,dc=org", "another-group-not-matched"},
	...
	userSearchConfig = ldap.ServerConfig{
		...
		Groups: []*ldap.GroupToOrgRole{
			...
			{
				GroupDN: "cn=admins2,ou=groups,dc=grafarg,dc=org",
				OrgId:   1,
				OrgRole: models.ROLE_ADMIN,
			},
		},
	...
	expected := `
		{
		  "name": {
			...
			"isDisabled": false,
			"roles": [
				{ "orgId": 1, "orgRole": "Admin", "orgName": "Main Org.", "groupDN": "cn=admins,ou=groups,dc=grafarg,dc=org" },
...
func TestPostSyncUserWithLDAPAPIEndpoint_WhenGrafargAdmin(t *testing.T) {
	sc := postSyncUserWithLDAPContext(t, "/api/admin/ldap/sync/34", func(t *testing.T, sc *scenarioContext) {
		getLDAPConfig = func(*setting.Cfg) (*ldap.Config, error) {
			return &ldap.Config{}, nil
		}
	...
	expected := `
	{
		...
		"message": "Refusing to sync grafarg super admin \"ldap-daniel\" - it would be disabled"
	}
	`
