...
import (
	...
	"github.com/famarker/grafarg/pkg/services/hooks"
...
func TestLoginErrorCookieAPIEndpoint(t *testing.T) {
	...
	cfg.LoginCookieName = "grafarg_session"
...
func TestLoginViewRedirect(t *testing.T) {
	...
	redirectCases := []redirectCase{
		{
			desc:        "grafarg relative url without subpath",
			url:         "/profile",
			redirectURL: "/profile",
		...
		{
			desc:        "grafarg invalid relative url starting with the subpath",
			url:         "/grafargblah",
			redirectURL: "/grafarg/",
			appURL:      "http://localhost:3000/",
			appSubURL:   "/grafarg",
		...
		{
			desc:        "grafarg relative url with subpath with leading slash",
			url:         "/grafarg/profile",
			redirectURL: "/grafarg/profile",
			appURL:      "http://localhost:3000",
			appSubURL:   "/grafarg",
			status:      302,
		},
		...
		{
			desc:        "non-Grafarg URL without scheme",
			url:         "example.com",
			redirectURL: "/",
			appURL:      "http://localhost:3000/",
			status:      302,
...
func TestLoginPostRedirect(t *testing.T) {
	...
	bus.AddHandler("grafarg-auth", func(query *models.LoginUserQuery) error {
	...
	redirectCases := []redirectCase{
		...
		{
			desc:      "grafarg invalid relative url starting with subpath",
			url:       "/grafargblah",
			appURL:    "https://localhost:3000/",
			appSubURL: "/grafarg",
...
func TestAuthProxyLoginWithEnableLoginToken(t *testing.T) {
	...
	assert.Equal(t, "grafarg_session=; Path=/; Max-Age=0; HttpOnly", setCookie[0])
}
...
func setupAuthProxyLoginTest(t *testing.T, enableLoginToken bool) *scenarioContext {
	...
	sc := setupScenarioContext(t, "/login")
	sc.cfg.LoginCookieName = "grafarg_session"
	hs := &HTTPServer{
		Cfg:              sc.cfg,
		License:          &licensing.OSSLicensingService{},
		AuthTokenService: auth.NewFakeUserAuthTokenService(),
