...
import (
	...
	"github.com/famarker/grafarg/pkg/services/live/features"
	"github.com/famarker/grafarg/pkg/setting"
	"github.com/famarker/grafarg/pkg/tsdb/cloudwatch"
)
...
func init() {
	registry.RegisterService(&GrafargLive{
...
func (g *GrafargLive) Init() error {
	logger.Debug("GrafargLive initialization")

	if !g.IsEnabled() {
		logger.Debug("GrafargLive feature not enabled, skipping initialization")
		return nil
	}
	...
	g.GrafargScope.Dashboards = dash
	g.GrafargScope.Features["dashboard"] = dash
	g.GrafargScope.Features["testdata"] = &features.TestDataSupplier{
		Publisher: g.Publish,
	}
	g.GrafargScope.Features["broadcast"] = &features.BroadcastRunner{}
	g.GrafargScope.Features["measurements"] = &features.MeasurementsRunner{}
...
func (g *GrafargLive) GetChannelHandlerFactory(scope string, name string) (models.ChannelHandlerFactory, error) {
	if scope == "grafarg" {
		p, ok := g.GrafargScope.Features[name]
		if ok {
			return p, nil
		}
