...
import (
	...
	"github.com/famarker/grafarg/pkg/events"
...
func GetSignedInUser(query *models.GetSignedInUserQuery) error {
	...
	var rawSQL = `SELECT
		u.id             as user_id,
		u.is_admin       as is_grafarg_admin,
		u.email          as email,
		u.login          as login,
		u.name           as name,
...
func UpdateUserPermissions(cmd *models.UpdateUserPermissionsCommand) error {
	return inTransaction(func(sess *DBSession) error {
		...
		user.IsAdmin = cmd.IsGrafargAdmin
		sess.UseBool("is_admin")
...
func validateOneAdminLeft(sess *DBSession) error {
	...
	if count == 0 {
		return models.ErrLastGrafargAdmin
	}
