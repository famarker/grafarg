...
func TestUserDataAccess(t *testing.T) {
	Convey("Testing DB", t, func() {
		...
		Convey("Given one grafarg admin user", func() {
			var err error
			...
			Convey("Cannot make themselves a non-admin", func() {
				updateUserPermsCmd := models.UpdateUserPermissionsCommand{IsGrafargAdmin: false, UserId: 1}
				updatePermsError := UpdateUserPermissions(&updateUserPermsCmd)

				So(updatePermsError, ShouldEqual, models.ErrLastGrafargAdmin)
