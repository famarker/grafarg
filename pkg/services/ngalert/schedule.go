...
func (sch *schedule) definitionRoutine(grafargCtx context.Context, key alertDefinitionKey, evalCh <-chan *evalContext, stopCh <-chan struct{}) error {
...
func (sch *schedule) Ticker(grafargCtx context.Context) error {
	dispatcherGroup, ctx := errgroup.WithContext(grafargCtx)
	for {
		select {
