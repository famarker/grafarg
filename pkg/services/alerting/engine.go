...
func (e *AlertEngine) alertingTicker(grafargCtx context.Context) error {
	defer func() {
	...
	for {
		select {
		case <-grafargCtx.Done():
			return grafargCtx.Err()
...
func (e *AlertEngine) runJobDispatcher(grafargCtx context.Context) error {
	dispatcherGroup, alertCtx := errgroup.WithContext(grafargCtx)
...
func (e *AlertEngine) processJobWithRetry(grafargCtx context.Context, job *Job) error {
	defer func() {
		if err := recover(); err != nil {
	...
	for {
		select {
		case <-grafargCtx.Done():
			// In case grafarg server context is cancel, let a chance to job processing
			...
			case <-unfinishedWorkTimer.C:
				return e.endJob(grafargCtx.Err(), cancelChan, job)
			case <-attemptChan:
