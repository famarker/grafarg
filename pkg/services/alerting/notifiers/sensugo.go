...
func (sn *SensuGoNotifier) Notify(evalContext *alerting.EvalContext) error {
	...
	// value (optional), else we fallback and use the grafarg rule anme  and ruleID.
	if sn.Entity != "" {
	...
	if sn.Check != "" {
		...
		bodyJSON.SetPath([]string{"check", "metadata", "name"}, "grafarg_rule_"+strconv.FormatInt(evalContext.Rule.ID, 10))
	}
