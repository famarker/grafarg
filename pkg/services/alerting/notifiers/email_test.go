	...
	Convey("Email notifier tests", t, func() {
		Convey("Parsing alert notification from settings", func() {
			...
			Convey("from settings", func() {
				...
				So(emailNotifier.Addresses[0], ShouldEqual, "ops@grafarg.org")
			})

			Convey("from settings with two emails", func() {
				json := `
				{
					"addresses": "ops@grafarg.org;dev@grafarg.org"
				}`
				...
				So(emailNotifier.Addresses[0], ShouldEqual, "ops@grafarg.org")
				So(emailNotifier.Addresses[1], ShouldEqual, "dev@grafarg.org")
			})
		})
	})
}
