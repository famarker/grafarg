...
func (sn *SlackNotifier) Notify(evalContext *alerting.EvalContext) error {
	...
	attachment := map[string]interface{}{
		...
		"fields":      fields,
		"footer":      "Grafarg v" + setting.BuildVersion,
		"footer_icon": "https://grafarg.com/assets/img/fav32.png",
		"ts":          time.Now().Unix(),
	}
...
func (sn *SlackNotifier) slackFileUpload(evalContext *alerting.EvalContext, log log.Logger, url string, recipient string, token string) error {
	if evalContext.ImageOnDiskPath == "" {
		// nolint:gosec
		// We can ignore the gosec G304 warning on this one because `setting.HomePath` comes from Grafarg's configuration file.
