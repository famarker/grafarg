...
func (sn *SensuNotifier) Notify(evalContext *alerting.EvalContext) error {
	...
	// else we fallback and use the grafarg ruleID.
	if sn.Source != "" {
		bodyJSON.Set("source", sn.Source)
	} else {
		bodyJSON.Set("source", "grafarg_rule_"+strconv.FormatInt(evalContext.Rule.ID, 10))
	}
	// Finally, sensu expects an output
	// We set it to a default output
	bodyJSON.Set("output", "Grafarg Metric Condition Met")
