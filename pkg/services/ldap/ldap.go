...
// 1. "admin" user
// Bind the "admin" user (defined in Grafarg config file) which has the search privileges
...
// 2. Single bind
// // If all the users meant to be used with Grafarg have the ability to search in LDAP server
...
func (server *Server) Login(query *models.LoginUserQuery) (
	*models.ExternalUserInfo, error,
	...
	user := users[0]
	if err := server.validateGrafargUser(user); err != nil {
		return nil, err
	}
...
// validateGrafargUser validates user access.
// If there are no ldap group mappings access is true
// otherwise a single group must match
func (server *Server) validateGrafargUser(user *models.ExternalUserInfo) error {
...
// buildGrafargUser extracts info from UserInfo model to ExternalUserInfo
func (server *Server) buildGrafargUser(user *ldap.Entry) (*models.ExternalUserInfo, error) {
	...
	for _, group := range server.Config.Groups {
		...
		if isMemberOf(memberOf, group.GroupDN) {
			...
			if extUser.IsGrafargAdmin == nil || !*extUser.IsGrafargAdmin {
				extUser.IsGrafargAdmin = group.IsGrafargAdmin
			}
		}
	}
...
func (server *Server) serializeUsers(
	...
	for _, user := range entries {
		extUser, err := server.buildGrafargUser(user)
		if err != nil {
			return nil, err
		}
