...
func (ls *LoginService) UpsertUser(cmd *models.UpsertUserCommand) error {
	...
	// Sync isGrafargAdmin permission
	if extUser.IsGrafargAdmin != nil && *extUser.IsGrafargAdmin != cmd.Result.IsAdmin {
		if err := ls.Bus.Dispatch(&models.UpdateUserPermissionsCommand{UserId: cmd.Result.Id, IsGrafargAdmin: *extUser.IsGrafargAdmin}); err != nil {
			return err
		}
	}
