...
func TestGetFullDashboardUrl(t *testing.T) {
	...
	setting.AppUrl = "http://grafarg.local/"
	url := GetFullDashboardUrl("uid", "my-dashboard")
	assert.Equal(t, "http://grafarg.local/d/uid/my-dashboard", url)
}

func TestDashboard_UpdateSlug(t *testing.T) {
	dashboard := NewDashboard("Grafarg Play Home")
	assert.Equal(t, "grafarg-play-home", dashboard.Slug)
...
func TestSlugifyTitle(t *testing.T) {
	testCases := map[string]string{
		"Grafarg Play Home": "grafarg-play-home",
