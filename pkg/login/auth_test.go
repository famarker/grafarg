...
func TestAuthenticateUser(t *testing.T) {
	authScenario(t, "When a user authenticates without setting a password", func(sc *authScenarioContext) {
		...
		mockLoginUsingGrafargDB(nil, sc)
		...
		assert.False(t, sc.grafargLoginWasCalled)
	...
	authScenario(t, "When grafarg user authenticate with valid credentials", func(sc *authScenarioContext) {
		mockLoginAttemptValidation(nil, sc)
		mockLoginUsingGrafargDB(nil, sc)
		...
		assert.True(t, sc.loginAttemptValidationWasCalled)
		assert.True(t, sc.grafargLoginWasCalled)
		assert.False(t, sc.ldapLoginWasCalled)
		assert.False(t, sc.saveInvalidLoginAttemptWasCalled)
		assert.Equal(t, "grafarg", sc.loginUserQuery.AuthModule)
	})

	authScenario(t, "When grafarg user authenticate and unexpected error occurs", func(sc *authScenarioContext) {
		customErr := errors.New("custom")
		mockLoginAttemptValidation(nil, sc)
		mockLoginUsingGrafargDB(customErr, sc)
		mockLoginUsingLDAP(true, ErrInvalidCredentials, sc)
		mockSaveInvalidLoginAttempt(sc)
	...
	authScenario(t, "When a non-existing grafarg user authenticate and ldap disabled", func(sc *authScenarioContext) {
		mockLoginAttemptValidation(nil, sc)
		mockLoginUsingGrafargDB(models.ErrUserNotFound, sc)
		mockLoginUsingLDAP(false, nil, sc)
		mockSaveInvalidLoginAttempt(sc)
	...
	authScenario(t, "When a non-existing grafarg user authenticate and invalid ldap credentials", func(sc *authScenarioContext) {
		mockLoginAttemptValidation(nil, sc)
		mockLoginUsingGrafargDB(models.ErrUserNotFound, sc)
	...
	authScenario(t, "When a non-existing grafarg user authenticate and valid ldap credentials", func(sc *authScenarioContext) {
		mockLoginAttemptValidation(nil, sc)
		mockLoginUsingGrafargDB(models.ErrUserNotFound, sc)
		mockLoginUsingLDAP(true, nil, sc)
		mockSaveInvalidLoginAttempt(sc)
	...
	authScenario(t, "When a non-existing grafarg user authenticate and ldap returns unexpected error", func(sc *authScenarioContext) {
		customErr := errors.New("custom")
		mockLoginAttemptValidation(nil, sc)
		mockLoginUsingGrafargDB(models.ErrUserNotFound, sc)
	...
	authScenario(t, "When grafarg user authenticate with invalid credentials and invalid ldap credentials", func(sc *authScenarioContext) {
		mockLoginAttemptValidation(nil, sc)
		mockLoginUsingGrafargDB(ErrInvalidCredentials, sc)
		mockLoginUsingLDAP(true, ldap.ErrInvalidCredentials, sc)
		mockSaveInvalidLoginAttempt(sc)
...
func mockLoginUsingGrafargDB(err error, sc *authScenarioContext) {
	loginUsingGrafargDB = func(query *models.LoginUserQuery) error {
		sc.grafargLoginWasCalled = true
		return err
	}
}
...
func authScenario(t *testing.T, desc string, fn authScenarioFunc) {
	...
	t.Run(desc, func(t *testing.T) {
		origLoginUsingGrafargDB := loginUsingGrafargDB
		...
		t.Cleanup(func() {
			loginUsingGrafargDB = origLoginUsingGrafargDB
