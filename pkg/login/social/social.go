...
const (
	grafargCom = "grafarg_com"
)

var (
	SocialBaseUrl = "/login/"
	SocialMap     = make(map[string]SocialConnector)
	allOauthes    = []string{"github", "gitlab", "google", "generic_oauth", "grafargnet", grafargCom, "azuread", "okta"}
)
	...
	for _, name := range allOauthes {
		...
		if name == grafargCom {
			config = oauth2.Config{
				...
				ClientSecret: info.ClientSecret,
				Endpoint: oauth2.Endpoint{
					AuthURL:   setting.GrafargComUrl + "/oauth2/authorize",
					TokenURL:  setting.GrafargComUrl + "/api/oauth2/token",
			...
			SocialMap[grafargCom] = &SocialGrafargCom{
				SocialBase:           newSocialBase(name, &config, info),
				url:                  setting.GrafargComUrl,
				allowedOrganizations: util.SplitString(sec.Key("allowed_organizations").String()),
			}
		}
	}
