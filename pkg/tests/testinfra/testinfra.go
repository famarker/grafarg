...
// StartGrafarg starts a Grafarg server.
// The server address is returned.
func StartGrafarg(t *testing.T, grafDir, cfgPath string, sqlStore *sqlstore.SQLStore) string {
	t.Helper()
	...
	t.Logf("Grafarg is listening on %s", addr)

	return addr
}

// SetUpDatabase sets up the Grafarg database.
...
// CreateGrafDir creates the Grafarg directory.
func CreateGrafDir(t *testing.T, opts ...GrafargOpts) (string, string) {
	t.Helper()
	...
	require.NoError(t, err)
	// Copy index template to index.html, since Grafarg will try to use the latter
	err = fs.CopyFile(filepath.Join(rootDir, "public", "views", "index-template.html"),
		filepath.Join(viewsDir, "index.html"))
	require.NoError(t, err)
	// Copy error template to error.html, since Grafarg will try to use the latter
