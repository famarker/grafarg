...
var (
	...
	// StatsTotalAnnotations is a metric of total number of annotations stored in Grafarg.
	StatsTotalAnnotations prometheus.Gauge

	// StatsTotalDashboardVersions is a metric of total number of dashboard versions stored in Grafarg.
	StatsTotalDashboardVersions prometheus.Gauge

	// grafargBuildVersion is a metric with a constant '1' value labeled by version, revision, branch, and goversion from which Grafarg was built
	grafargBuildVersion *prometheus.GaugeVec

	grafargPluginBuildInfoDesc *prometheus.GaugeVec
)
...
func init() {
	...
	grafargBuildVersion = prometheus.NewGaugeVec(prometheus.GaugeOpts{
		Name:      "build_info",
		Help:      "A metric with a constant '1' value labeled by version, revision, branch, and goversion from which Grafarg was built",
		Namespace: ExporterName,
	}, []string{"version", "revision", "branch", "goversion", "edition"})

	grafargPluginBuildInfoDesc = prometheus.NewGaugeVec(prometheus.GaugeOpts{
		Name:      "plugin_build_info",
		Help:      "A metric with a constant '1' value labeled by pluginId, pluginType and version from which Grafarg plugin was built",
		Namespace: ExporterName,
...
func SetBuildInformation(version, revision, branch string) {
	...
	grafargBuildVersion.WithLabelValues(version, revision, branch, runtime.Version(), edition).Set(1)
}
...
func SetEnvironmentInformation(labels map[string]string) error {
	...
	grafargEnvironmentInfo := prometheus.NewGauge(prometheus.GaugeOpts{
	...
	prometheus.MustRegister(grafargEnvironmentInfo)

	grafargEnvironmentInfo.Set(1)
	return nil
}

func SetPluginBuildInformation(pluginID, pluginType, version string) {
	grafargPluginBuildInfoDesc.WithLabelValues(pluginID, pluginType, version).Set(1)
}
