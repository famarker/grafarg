...
// getGrafargRoot tries to get root of directory when developing grafarg, ie. repo root. It is not perfect, it just
// checks what is the binary path and tries to guess based on that, but if it is not running in dev env you get a bogus
// path back.
func getGrafargRoot() (string, error) {
...
func returnOsDefault(currentOs string) string {
	...
	case "windows":
		return "../data/plugins"
	case "darwin":
		return "/usr/local/var/lib/grafarg/plugins"
	case "freebsd":
		return "/var/db/grafarg/plugins"
	case "openbsd":
		return "/var/grafarg/plugins"
	default: // "linux"
		return "/var/lib/grafarg/plugins"
	}
}
