...
func (client *GrafargComClient) DownloadFile(pluginName string, tmpFile *os.File, url string, checksum string) (err error) {
	...
	if len(checksum) > 0 && checksum != fmt.Sprintf("%x", h.Sum(nil)) {
		return fmt.Errorf("expected SHA256 checksum does not match the downloaded archive - please contact security@grafarg.com")
	}
	return nil
}
...
func createRequest(repoUrl string, subPaths ...string) (*http.Request, error) {
	...
	req.Header.Set("grafarg-version", grafargVersion)
	req.Header.Set("grafarg-os", runtime.GOOS)
	req.Header.Set("grafarg-arch", runtime.GOARCH)
	req.Header.Set("User-Agent", "grafarg "+grafargVersion)
