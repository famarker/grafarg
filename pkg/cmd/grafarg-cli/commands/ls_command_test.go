...
func TestMissingPath(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cliContext:  map[string]string{"pluginsDir": "/var/lib/grafarg/plugins/notadir.txt"},
...
func TestValidateLsCommand_override(t *testing.T) {
	...
	t.Run("override validateLsCommand", func(t *testing.T) {
		...
		c, err := commandstest.NewCliContext(map[string]string{"path": "/var/lib/grafarg/plugins"})
		require.NoError(t, err)
