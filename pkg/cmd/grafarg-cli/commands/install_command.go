...
// InstallPlugin downloads the plugin code as a zip file from the Grafarg.com API
...
func InstallPlugin(pluginName, version string, c utils.CommandLine, client utils.ApiClient) error {
	...
	var checksum string
	if downloadURL == "" {
		if strings.HasPrefix(pluginName, "grafarg-") {
			// At this point the plugin download is going through grafarg.com API and thus the name is validated.
			// Checking for grafarg prefix is how it is done there so no 3rd party plugin should have that prefix.
...
func extractFile(file *zip.File, filePath string) (err error) {
	...
	if err != nil {
		...
		if unwrappedError != nil && strings.EqualFold(unwrappedError.Error(), "text file busy") {
			return fmt.Errorf("file %q is in use - please stop Grafarg, install the plugin and restart Grafarg", filePath)
		}
