...
func TestExtractFiles(t *testing.T) {
	t.Run("Should preserve file permissions for plugin backend binaries for linux and darwin", func(t *testing.T) {
		...
		archive := filepath.Join("testdata", "grafarg-simple-json-datasource-ec18fa4da8096a952608a7e4c7782b4260b41bcf.zip")
		err := extractFiles(archive, "grafarg-simple-json-datasource", pluginsDir, false)
		require.NoError(t, err)

		// File in zip has permissions 755
		fileInfo, err := os.Stat(filepath.Join(pluginsDir, "grafarg-simple-json-datasource",
			"simple-plugin_darwin_amd64"))
		require.NoError(t, err)
		assert.Equal(t, "-rwxr-xr-x", fileInfo.Mode().String())

		// File in zip has permission 755
		fileInfo, err = os.Stat(pluginsDir + "/grafarg-simple-json-datasource/simple-plugin_linux_amd64")
		require.NoError(t, err)
		assert.Equal(t, "-rwxr-xr-x", fileInfo.Mode().String())

		// File in zip has permission 644
		fileInfo, err = os.Stat(pluginsDir + "/grafarg-simple-json-datasource/simple-plugin_windows_amd64.exe")
		require.NoError(t, err)
		assert.Equal(t, "-rw-r--r--", fileInfo.Mode().String())

		// File in zip has permission 755
		fileInfo, err = os.Stat(pluginsDir + "/grafarg-simple-json-datasource/non-plugin-binary")
		require.NoError(t, err)
		assert.Equal(t, "-rwxr-xr-x", fileInfo.Mode().String())
	})
...
func TestInstallPluginCommand(t *testing.T) {
	...
	client := &commandstest.FakeGrafargComClient{
		...
		DownloadFileFunc: func(pluginName string, tmpFile *os.File, url string, checksum string) (err error) {
			...
			f, err := os.Open("testdata/grafarg-simple-json-datasource-ec18fa4da8096a952608a7e4c7782b4260b41bcf.zip")
			require.NoError(t, err)
