...
func main() {
	...
	app := &cli.App{
		...
		Authors: []*cli.Author{
			{
				Name:  "Grafarg Project",
				Email: "hello@grafarg.com",
			},
		},
		Version: version,
		Flags: []cli.Flag{
			&cli.StringFlag{
				Name:    "pluginsDir",
				Usage:   "Path to the Grafarg plugin directory",
				Value:   utils.GetGrafargPluginDir(runtime.GOOS),
				EnvVars: []string{"GF_PLUGIN_DIR"},
			},
			&cli.StringFlag{
				Name:    "repo",
				Usage:   "URL to the plugin repository",
				Value:   "https://grafarg.com/api/plugins",
				EnvVars: []string{"GF_PLUGIN_REPO"},
			},
			&cli.StringFlag{
				Name:    "pluginUrl",
				Usage:   "Full url to the plugin zip file instead of downloading the plugin from grafarg.com/api",
				Value:   "",
			...
			&cli.StringFlag{
				Name:  "homepath",
				Usage: "Path to Grafarg install/home path, defaults to working directory",
			},
			&cli.StringFlag{
				Name:  "config",
