...
func (cfg *Cfg) readGrafargEnvironmentMetrics() error {
	environmentMetricsSection := cfg.Raw.Section("metrics.environment_info")
	keys := environmentMetricsSection.Keys()
	cfg.MetricsGrafargEnvironmentInfo = make(map[string]string, len(keys))
	...
	for _, key := range keys {
		...
		cfg.MetricsGrafargEnvironmentInfo[string(labelName)] = string(labelValue)
	}
...
func (cfg *Cfg) loadConfiguration(args *CommandLineArgs) (*ini.File, error) {
	...
	if _, err := os.Stat(defaultConfigFile); os.IsNotExist(err) {
		fmt.Println("Grafarg-server Init Failed: Could not find config defaults, make sure homepath command line parameter is set or working directory is homepath")
		os.Exit(1)
	}
...
func (cfg *Cfg) Load(args *CommandLineArgs) error {
	...
	ReportingDistributor = analytics.Key("reporting_distributor").MustString("grafarg-labs")
	...
	cfg.MarketplaceURL = pluginsSection.Key("marketplace_url").MustString("https://grafarg.com/grafarg/plugins/")
	...
	if err := cfg.readGrafargEnvironmentMetrics(); err != nil {
		return err
	}
	...
	// check old key  name
	GrafargComUrl = valueAsString(iniFile.Section("grafarg_net"), "url", "")
	if GrafargComUrl == "" {
		GrafargComUrl = valueAsString(iniFile.Section("grafarg_com"), "url", "https://grafarg.com")
	}
...
func readAuthSettings(iniFile *ini.File, cfg *Cfg) (err error) {
	...
	cfg.LoginCookieName = valueAsString(auth, "login_cookie_name", "grafarg_session")
