...
description = "Guide for using Microsoft SQL Server in Grafarg"
keywords = ["grafarg", "MSSQL", "Microsoft", "SQL", "guide", "Azure SQL Database"]
aliases = ["/docs/grafarg/latest/features/datasources/mssql/"]
weight = 900
+++

# Using Microsoft SQL Server in Grafarg

Grafarg ships with a built-in Microsoft SQL Server (MS SQL) data source plugin that allows you to query and visualize data from any Microsoft SQL Server 2005 or newer, including Microsoft Azure SQL Database. This topic explains options, variables, querying, and other options specific to the MS SQL data source. Refer to [Add a data source]({{< relref "add-a-data-source.md" >}}) for instructions on how to add a data source to Grafarg. Only users with the organization admin role can add data sources.
...
```sql
 ...
 GRANT SELECT ON dbo.YourTable3 TO grafargreader
```
...
| `$__timeGroupAlias(dateColumn,'5m')`                  | Will be replaced identical to \$\_\_timeGroup but with an added column alias (only available in Grafarg 5.3+).                                                                                                                                                                              |
...
>
> From Grafarg 5.0.0, template variable values are only quoted when the template variable is a `multi-value`.
