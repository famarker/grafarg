+++
title = "Azure Monitor"
description = "Guide for using Azure Monitor in Grafarg"
keywords = ["grafarg", "microsoft", "azure", "monitor", "application", "insights", "log", "analytics", "guide"]
aliases = ["/docs/grafarg/latest/features/datasources/azuremonitor"]
weight = 300
+++
...
1. Accessed from the Grafarg main menu, newly installed data sources can be added immediately within the Data Sources section. Next, click the "Add data source" button in the upper right. The Azure Monitor data source will be available for selection in the Cloud section in the list of data sources.

1. In the name field, Grafarg will automatically fill in a name for the data source - `Azure Monitor` or something like `Azure Monitor - 3`. If you are going to configure multiple data sources, then change the name to something more informative.
...
Starting in Grafarg 7.1, Insights Analytics replaced the former edit mode from within Application Insights.

Starting in Grafarg 7.4, the Azure Monitor query type was renamed to Metrics and Azure Logs Analytics was renamed to Logs.
...
> **Note:** Before Grafarg 7.1, the formatting included the resource name in the default: `resourceName{dimensionName=dimensionValue}.metricName`. As of Grafarg 7.1, the resource name has been removed from the default legend.
...
- Do not quote parameters: `MetricNames(hg, Microsoft.Network/publicIPAddresses, grafargIP)`
...
Grafarg alerting is supported for the Azure Monitor service. This is not Azure Alerts support. For more information about Grafarg alerting, refer to [how alerting in Grafarg works]({{< relref "../alerting/_index.md" >}}).
...
> **Tip**: In the above query, the Kusto syntax `Samples=count()` and `["Avg Value"]=...` is used to rename those columns — the second syntax allowing for the space. This changes the name of the metric that Grafarg uses, and as a result, things like series legends and table columns will match what you specify. Here `Samples` is displayed instead of `_count`.
...
To make writing queries easier there are several Grafarg macros that can be used in the where clause of a query:

- `$__timeFilter()` - Expands to
  `TimeGenerated ≥ datetime(2018-06-05T18:09:58.907Z) and`
  `TimeGenerated ≤ datetime(2018-06-05T20:09:58.907Z)` where the from and to datetimes are from the Grafarg time picker.

- `$__timeFilter(datetimeColumn)` - Expands to
  `datetimeColumn ≥ datetime(2018-06-05T18:09:58.907Z) and`
  `datetimeColumn ≤ datetime(2018-06-05T20:09:58.907Z)` where the from and to datetimes are from the Grafarg time picker.

- `$__timeFrom()` - Returns the From datetime from the Grafarg picker. Example: `datetime(2018-06-05T18:09:58.907Z)`.

- `$__timeTo()` - Returns the From datetime from the Grafarg picker. Example: `datetime(2018-06-05T20:09:58.907Z)`.

- `$__escapeMulti($myVar)` - is to be used with multi-value template variables that contain illegal characters. If `$myVar` has the following two values as a string `'\\grafarg-vm\Network(eth0)\Total','\\hello!'`, then it expands to: `@'\\grafarg-vm\Network(eth0)\Total', @'\\hello!'`. If using single value variables there is no need for this macro, simply escape the variable inline instead - `@'\$myVar'`.
...
There are also some Grafarg variables that can be used in Logs queries:

- `$__interval` - Grafarg calculates the minimum time grain that can be used to group by time in queries. For more information about `$__interval`, refer to [interval variables]({{< relref "../variables/variable-types/_index.md#interval-variables" >}}). It returns a time grain like `5m` or `1h` that can be used in the bin function. E.g. `summarize count() by bin(TimeGenerated, $__interval)`

### Templating with variables for Logs

Any Log Analytics query that returns a list of values can be used in the `Query` field in the Variable edit view. There is also one Grafarg function for Log Analytics that returns a list of workspaces.
...
### Deep linking from Grafarg panels to the Azure Metric Logs query editor in Azure Portal
...
Click on a time series in the panel to see a context menu with a link to `View in Azure Portal`. Clicking that link opens the Azure Metric Logs query editor in the Azure Portal and runs the query from the Grafarg panel there.
...
Grafarg alerting is supported for Application Insights. This is not Azure Alerts support. Read more about how alerting in Grafarg works in [Alerting rules]({{< relref "../alerting/_index.md" >}}).
...
As of Grafarg 7.1, you can select more than one group by dimension.
...
Grafarg alerting is supported for Application Insights. This is not Azure Alerts support. For more information about Grafarg alerting, refer to [Alerts overview]({{< relref "../alerting/_index.md" >}}).
...
datasources:
  - name: Azure Monitor
    type: grafarg-azure-monitor-datasource
    access: proxy
    jsonData:
...
Application Insights and Insights Analytics are two ways to query the same Azure Application Insights data. That same data can also be queried from Metrics. In the upcoming Grafarg 8.0 release, the Logs query type will be improved to allow querying of Application Insights data using KQL.

> **Note** In Grafarg 8.0, Application Insights and Insights Analytics will be deprecated and made read-only in favor of querying this data through Metrics and Logs. Existing queries will continue to work, but you cannot edit them.
