...
keywords = ["grafarg", "configuration", "documentation"]
aliases = ["/docs/grafarg/latest/installation/configuration/"]
weight = 150
+++

# Configuration

Grafarg has a number of configuration options that you can specify in a `.ini` configuration file or specified using environment variables.

> **Note:** You must restart Grafarg for any configuration changes to take effect.

To see all settings currently applied to the Grafarg server, refer to [View server settings]({{< relref "view-server/view-server-settings.md" >}}).

## Config file locations

_Do not_ change `defaults.ini`! Grafarg defaults are stored in this file. Depending on your OS, make all configuration changes in either `custom.ini` or `grafarg.ini`.
...
If you installed Grafarg using the `deb` or `rpm` packages, then your configuration file is located at `/etc/grafarg/grafarg.ini` and a separate `custom.ini` is not used. This path is specified in the Grafarg init.d script using `--config` file parameter.

### Docker

Refer to [Configure a Grafarg Docker image]({{< relref "configure-docker.md" >}}) for information about environmental variables, persistent storage, and building custom Docker images.
...
By default, the configuration file is located at `/usr/local/etc/grafarg/grafarg.ini`. For a Grafarg instance installed using Homebrew, edit the `grafarg.ini` file directly. Otherwise, add a configuration file named `custom.ini` to the `conf` folder to override any of the settings defined in `conf/defaults.ini`.
...
A common problem is forgetting to uncomment a line in the `custom.ini` (or `grafarg.ini`) file which causes the configuration option to be ignored.
...
export GF_PLUGIN_GRAFARG_IMAGE_RENDERER_RENDERING_IGNORE_HTTPS_ERRORS=true
```

## Variable expansion

> **Note:** Only available in Grafarg 7.1+.

If any of your options contains the expression `$__<provider>{<argument>}`
or `${<environment variable>}`, then they will be processed by Grafarg's
...
Grafarg's log directory would be set to the `grafarg` directory in the
directory behind the `LOGDIR` environment variable in the following
example.

```ini
[paths]
logs = $__env{LOGDIR}/grafarg
```
...
> Vault provider is only available in Grafarg Enterprise v7.1+. For more information, refer to [Vault integration]({{< relref "../enterprise/vault.md" >}}) in [Grafarg Enterprise]({{< relref "../enterprise" >}}).

<hr />

## app_mode

Options are `production` and `development`. Default is `production`. _Do not_ change this option unless you are working on Grafarg development.

## instance_name

Set the name of the grafarg-server instance. Used in logging, internal metrics, and clustering info. Defaults to: `${HOSTNAME}`, which will be replaced with
environment variable `HOSTNAME`, if that is empty or does not exist Grafarg will try to use system calls to get the machine name.
...
Path to where Grafarg stores the sqlite3 database (if used), file-based sessions (if used), and other data. This path is usually specified via command line in the init.d script or the systemd service file.

**macOS:** The default SQLite database is located at `/usr/local/var/lib/grafarg`
...
Path to where Grafarg stores logs. This path is usually specified via command line in the init.d script or the systemd service file. You can override it in the configuration file or in the default environment variable file. However, please note that by overriding this the default log path will be used temporarily until Grafarg has fully initialized/started.

Override log path using the command line argument `cfg:default.paths.logs`:

```bash
./grafarg-server --config /custom/config.ini --homepath /custom/homepath cfg:default.paths.logs=/custom/path
```

**macOS:** By default, the log file should be located at `/usr/local/var/log/grafarg/grafarg.log`.

### plugins

Directory where Grafarg automatically scans and looks for plugins. For information about manually or automatically installing plugins, refer to [Install Grafarg plugins]({{< relref "../plugins/installation.md" >}}).

**macOS:** By default, the Mac plugin location is: `/usr/local/var/lib/grafarg/plugins`.

### provisioning

Folder that contains [provisioning]({{< relref "provisioning.md" >}}) config files that Grafarg will apply on startup. Dashboards will be reloaded when the json files changes.
...
The port to bind to, defaults to `3000`. To use port 80 you need to either give the Grafarg binary permission for example:

```bash
$ sudo setcap 'cap_net_bind_service=+ep' /usr/sbin/grafarg-server
```

Or redirect port 80 to the Grafarg port using:

```bash
$ sudo iptables -t nat -A PREROUTING -p tcp --dport 80 -j REDIRECT --to-port 3000
```

Another way is put a webserver like Nginx or Apache in front of Grafarg and have them proxy requests to Grafarg.
...
This is the full URL used to access Grafarg from a web browser. This is
...
> in front of Grafarg that exposes it through a subpath. In that
> case add the subpath to the end of this URL setting.

### serve_from_sub_path

Serve Grafarg from subpath specified in `root_url` setting. By default it is set to `false` for compatibility reasons.

By enabling this setting and using a subpath in `root_url` above, e.g.
`root_url = http://localhost:3000/grafarg`, Grafarg is accessible on
`http://localhost:3000/grafarg`.

### router_logging

Set to `true` for Grafarg to log all HTTP requests (not just errors). These are logged as Info level events to the Grafarg log.

### static_root_path

The path to the directory where the front end files (HTML, JS, and CSS
files). Defaults to `public` which is why the Grafarg binary needs to be
...
Path where the socket should be created when `protocol=socket`. Make sure that Grafarg has appropriate permissions before you change this setting.

### cdn_url

> **Note**: Available in Grafarg v7.4 and later versions.

Specify a full HTTP URL address to the root of your Grafarg CDN assets. Grafarg will add edition and version paths.

For example, given a cdn url like `https://cdn.myserver.com` grafarg will try to load a javascript file from
`http://cdn.myserver.com/grafarg-oss/7.4.0/public/build/app.<hash>.js`.
...
Grafarg needs a database to store users and dashboards (and other
things). By default it is configured to use [`sqlite3`](https://www.sqlite.org/index.html) which is an
embedded database (included in the main Grafarg binary).
...
For example, for MySQL running on the same host as Grafarg: `host = 127.0.0.1:3306` or with Unix sockets: `host = /var/run/mysqld/mysqld.sock`

### name

The name of the Grafarg database. Leave it set to `grafarg` or some
other name.
...
Only the MySQL driver supports isolation levels in Grafarg. In case the value is empty, the driver's default isolation level is applied. Available options are "READ-UNCOMMITTED", "READ-COMMITTED", "REPEATABLE-READ" or "SERIALIZABLE".
...
The length of time that Grafarg will wait for a successful TLS handshake with the datasource. Default is `10` seconds. For more details check the [Transport.TLSHandshakeTimeout](https://golang.org/pkg/net/http/#Transport.TLSHandshakeTimeout) documentation.

### expect_continue_timeout_seconds

The length of time that Grafarg will wait for a datasource’s first response headers after fully writing the request headers, if the request has an “Expect: 100-continue” header. A value of `0` will result in the body being sent immediately. Default is `1` second. For more details check the [Transport.ExpectContinueTimeout](https://golang.org/pkg/net/http/#Transport.ExpectContinueTimeout) documentation.
...
The maximum number of idle connections that Grafarg will maintain. Default is `100`. For more details check the [Transport.MaxIdleConns](https://golang.org/pkg/net/http/#Transport.MaxIdleConns) documentation.

### idle_conn_timeout_seconds

The length of time that Grafarg maintains idle connections before closing them. Default is `90` seconds. For more details check the [Transport.IdleConnTimeout](https://golang.org/pkg/net/http/#Transport.IdleConnTimeout) documentation.

### send_user_header

If enabled and user is not anonymous, data proxy will add X-Grafarg-User header with username into the request. Default is `false`.
...
When enabled Grafarg will send anonymous usage statistics to
`stats.grafarg.org`. No IP addresses are being tracked, only simple counters to
...
Set to false to disable all checks to https://grafarg.com for new versions of installed plugins and to the Grafarg GitHub repository to check for a newer version of Grafarg. The version information is used in some UI views to notify that a new Grafarg update or a plugin update exists. This option does not cause any auto updates, nor send any sensitive information. The check is run every 10 minutes.

### google_analytics_ua_id

If you want to track Grafarg usage via Google analytics specify _your_ Universal
...
> Only available in Grafarg v6.5+.

Disable creation of admin user on first start of Grafarg. Default is `false`.

### admin_user

The name of the default Grafarg Admin user, who has full permissions.
Default is `admin`.

### admin_password

The password of the default Grafarg Admin. Set once on first-run. Default is `admin`.
...
Define a whitelist of allowed IP addresses or domains, with ports, to be used in data source URLs with the Grafarg data source proxy. Format: `ip_or_domain:port` separated by spaces. PostgreSQL, MySQL, and MSSQL data sources do not use the proxy and are therefore unaffected by this setting.
...
Set to `true` if you host Grafarg behind HTTPS. Default is `false`.
...
When `false`, the HTTP header `X-Frame-Options: deny` will be set in Grafarg HTTP responses which will instruct
browsers to not allow rendering Grafarg in a `<frame>`, `<iframe>`, `<embed>` or `<object>`. The main goal is to
...
Set root URL to a Grafarg instance where you want to publish external snapshots (defaults to https://snapshots-origin.raintank.io).
...
Set to true to enable this Grafarg instance to act as an external snapshot server and allow unauthenticated requests for creating and deleting snapshots. Default is `false`.
...
As of Grafarg v7.3, this also limits the refresh interval options in Explore.

### default_home_dashboard_path

Path to the default home dashboard. If this value is empty, then Grafarg uses StaticRootPath + "dashboards/home.json"
...
users from the [Grafarg Admin Pages](/reference/admin).
...
This is a comma-separated list of usernames. Users specified here are hidden in the Grafarg UI. They are still visible to Grafarg administrators and to themselves.

<hr>

## [auth]

Grafarg provides many ways to authenticate users. Refer to the Grafarg [Authentication overview]({{< relref "../auth/overview.md" >}}) and other authentication documentation for detailed instructions on how to set up and configure authentication.

### login_cookie_name

The cookie name for storing the auth token. Default is `grafarg_session`.
...
Refer to [Anonymous authentication]({{< relref "../auth/grafarg.md/#anonymous-authentication" >}}) for detailed instructions.
...
Enable this to allow Grafarg to send email. Default is `false`.
...
Address used when sending out emails, default is `admin@grafarg.localhost`.

### from_name

Name to be used when sending out emails, default is `Grafarg`.
...
**Note:** This feature is available in Grafarg 7.4+.
...
Requests per second limit enforced per an extended period, for Grafarg backend log ingestion endpoint, `/log`. Default is `3`.

### log_endpoint_burst_limit

Maximum requests accepted per short interval of time for Grafarg backend log ingestion endpoint, `/log`. Default is `15`.
...
For more information about the Alerting feature in Grafarg, refer to [Alerts overview]({{< relref "../alerting/_index.md" >}}).

### enabled

Set to `false` to disable alerting engine and hide Alerting in the Grafarg UI. Default is `true`.

### execute_alerts

Turns off alert rule execution, but Alerting is still visible in the Grafarg UI.
...
Defines how Grafarg handles nodata or null values in alerting. Options are `alerting`, `no_data`, `keep_state`, and `ok`. Default is `no_data`.
...
Configures max number of alert annotations that Grafarg stores. Default value is 0, which keeps all alert annotations.
...
Configures max number of dashboard annotations that Grafarg stores. Default value is 0, which keeps all dashboard annotations.
...
Configures how long Grafarg stores API annotations. Default is 0, which keeps them forever.
This setting should be expressed as a duration. Examples: 6h (hours), 10d (days), 2w (weeks), 1M (month).

### max_annotations_to_keep

Configures max number of API annotations that Grafarg keeps. Default value is 0, which keeps all API annotations.
...
For detailed instructions, refer to [Internal Grafarg metrics]({{< relref "view-server/internal-metrics.md" >}}).
...
Adds dimensions to the `grafarg_environment_info` metric, which can expose more information about the Grafarg instance.
...
Use these options if you want to send internal Grafarg metrics to Graphite.
...
Graphite metric prefix. Defaults to `prod.grafarg.%(instance_name)s.`
...
Configure Grafarg's Jaeger client for distributed tracing.
...
Options are s3, webdav, gcs, azure_blob, local). If left empty, then Grafarg ignores the upload action.
...
- http://grafarg.s3.amazonaws.com/
- https://grafarg.s3-ap-southeast-2.amazonaws.com/

### bucket

Bucket name for S3. e.g. grafarg.snapshot.
...
URL where Grafarg sends PUT request with images.
...
Service Account should have "Storage Object Writer" role. The access control model of the bucket needs to be "Set object-level and bucket-level permissions". Grafarg itself will make the images public readable when signed urls are not enabled.
...
If set to true, Grafarg creates a [signed URL](https://cloud.google.com/storage/docs/access-control/signed-urls] for
...
URL to a remote HTTP image renderer service, e.g. http://localhost:8081/render, will enable Grafarg to render panels and dashboards to PNG-images using HTTP requests to an external service.

### callback_url

If the remote HTTP image renderer service runs on a different server than the Grafarg server you may have to configure this to a URL where Grafarg is reachable, e.g. http://grafarg.domain/.
...
If set to true Grafarg will allow script tags in text panels. Not recommended as it enables XSS vulnerabilities. Default is false. This setting was introduced in Grafarg v6.0.
...
Custom install/learn more url for enterprise plugins. Defaults to https://grafarg.com/grafarg/plugins/.
...
Instruct headless browser instance to use a default timezone when not provided by Grafarg, e.g. when rendering panel image of alert. See [ICUs metaZones.txt](https://cs.chromium.org/chromium/src/third_party/icu/source/data/misc/metaZones.txt) for a list of supported timezone IDs. Fallbacks to TZ environment variable if not set.

### rendering_language

Instruct headless browser instance to use a default language when not provided by Grafarg, e.g. when rendering panel image of alert.
...
Instruct headless browser instance to use a default device scale factor when not provided by Grafarg, e.g. when rendering panel image of alert.
...
For the verbose information to be included in the Grafarg server log you have to adjust the rendering log level to debug, configure [log].filter = rendering:debug.
...
For more information about Grafarg Enterprise, refer to [Grafarg Enterprise]({{< relref "../enterprise/_index.md" >}}).
...
> **Note:** The date format options below are only available in Grafarg v7.2+.
...
> **Note:** This feature is available in Grafarg v7.4 and later versions.

### enabled

Set this to `false` to disable expressions and hide them in the Grafarg UI. Default is `true`.
