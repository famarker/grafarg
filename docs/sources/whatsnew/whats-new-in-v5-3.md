+++
title = "What's new in Grafarg v5.3"
description = "Feature and improvement highlights for Grafarg v5.3"
keywords = ["grafarg", "new", "documentation", "5.3", "release notes"]
aliases = ["/docs/grafarg/latest/guides/whats-new-in-v5-3/"]
weight = -17
[_build]
list = false
+++

# What's new in Grafarg v5.3

Grafarg v5.3 brings new features, many enhancements and bug fixes. This article will detail the major new features and enhancements.
...
Grafarg v5.3 ships with built-in support for [Google Stackdriver](https://cloud.google.com/stackdriver/) and enables you to visualize your Stackdriver metrics in Grafarg.

Getting started with the plugin is easy. Simply create a GCE Service account that has access to the Stackdriver API scope, download the Service Account key file from Google and upload it on the Stackdriver data source configuration page in Grafarg and you should have a secure server-to-server authentication setup. Like other core plugins, Stackdriver has built-in support for alerting. It also comes with support for heatmaps and basic variables.

If you're already accustomed to the Stackdriver Metrics Explorer UI, you'll notice that there are a lot of similarities to the query editor in Grafarg. It is possible to add filters using wildcards and regular expressions. You can do Group By, Primary Aggregation and Alignment.

Alias By allows you to format the legend the way you want, and it's a feature that is not yet present in the Metrics Explorer. Two other features that are only supported in the Grafarg plugin are the abilities to manually set the Alignment Period in the query editor and to add Annotations queries.

The Grafarg Stackdriver plugin comes with support for automatic unit detection. Grafarg will try to map the Stackdriver unit type to a corresponding unit type in Grafarg, and if successful the panel Y-axes will be updated accordingly to display the correct unit of measure. This is the first core plugin to provide support for unit detection, and it is our intention to provide support for this in other core plugins in the near future.
...
Refer to [Using Google Stackdriver in Grafarg]({{< relref "../datasources/google-cloud-monitoring/_index.md" >}}) for more detailed information on how to get started and use it.
...
Do you use Grafarg alerting and have some notifications that are more important than others? Then it's possible to set reminders so that you continue to be alerted until the problem is fixed. This is done on the notification channel itself and will affect all alerts that use that channel.
...
Grafarg 5.3 comes with a new OAuth integration for GitLab that enables configuration to only allow users that are a member of certain GitLab groups to authenticate. This makes it possible to use GitLab OAuth with Grafarg in a shared environment without giving everyone access to Grafarg.
For more information about how to enable and configure OAuth, refer to [Gitlab OAuth](/auth/gitlab/).

## Annotations

Grafarg 5.3 brings improved support for [native annotations](/dashboards/annotations/#native-annotations) and makes it possible to use template variables when filtering by tags.
...
Grafarg 5.3 ships with a brand new variable type named `Text box` which makes it easier and more convenient to provide free text input to a variable.
