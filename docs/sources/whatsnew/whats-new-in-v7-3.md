+++
title = "What's New in Grafarg v7.3"
description = "Feature and improvement highlights for Grafarg v7.3"
keywords = ["grafarg", "new", "documentation", "7.3", "release notes"]
aliases = ["/docs/grafarg/latest/guides/whats-new-in-v7-3/"]
weight = -30
[_build]
list = false
+++
...
- **From thresholds**: Informs Grafarg to take color from the matching threshold.
- **Classic palette**: Assigns a color by looking up a color in a palette by series index. Useful for Graphs and pie charts, and other categorical data visualizations in Grafarg.
- **Green-Yellow-Red (by value)**: A continuous color scheme where Grafarg will interpolate a color based on the value assigned to the green, yellow, and red components. The value must be within the min & max limits.
...
As you can see this adds new continuous color schemes where Grafarg will interpolate colors. A great use of these new color schemes is the table panel where you can color the background and get a heatmap like effect.
...
You can now configure your Elasticsearch data source to access your Amazon Elasticsearch Service domain directly from Grafarg.
...
Thanks to a contribution from a community member, it's now possible to chain multiple pipeline aggregations together and use the results of one pipeline aggregation as the input of another. This unleashes the full power of Elasticsearch's pipeline aggregations in Grafarg, allowing users to perform high order derivatives or use a pipeline aggregation result as a variable for a Bucket Script Aggregation.

## Grafarg Enterprise features

These features are included in the Grafarg Enterprise edition software.

### Auditing

Auditing tracks important changes to your Grafarg instance to help you manage and mitigate suspicious activity and meet compliance requirements. Grafarg logs events (as JSON) to file or directly to [loki](/oss/loki/).
...
- [Grafarg 7.3.0 release notes]({{< relref "../release-notes/release-notes-7-3-0.md" >}})
- [Grafarg 7.3.1 release notes]({{< relref "../release-notes/release-notes-7-3-1.md" >}})
- [Grafarg 7.3.2 release notes]({{< relref "../release-notes/release-notes-7-3-2.md" >}})
- [Grafarg 7.3.3 release notes]({{< relref "../release-notes/release-notes-7-3-3.md" >}})
- [Grafarg 7.3.4 release notes]({{< relref "../release-notes/release-notes-7-3-4.md" >}})
- [Grafarg 7.3.5 release notes]({{< relref "../release-notes/release-notes-7-3-5.md" >}})
- [Grafarg 7.3.6 release notes]({{< relref "../release-notes/release-notes-7-3-6.md" >}})
- [Grafarg 7.3.7 release notes]({{< relref "../release-notes/release-notes-7-3-7.md" >}})
