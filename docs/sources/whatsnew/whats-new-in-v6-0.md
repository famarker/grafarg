+++
title = "What's new in Grafarg v6.0"
description = "Feature and improvement highlights for Grafarg v6.0"
keywords = ["grafarg", "new", "documentation", "6.0", "release notes"]
aliases = ["/docs/grafarg/latest/guides/whats-new-in-v6-0/"]
weight = -19
[_build]
list = false
+++

# What's new in Grafarg v6.0

This update to Grafarg introduces a new way of exploring your data, support for log data, and tons of other features.

The main highlights are:

- [Explore]({{< relref "#explore" >}}) - A new query focused workflow for ad-hoc data exploration and troubleshooting.
- [Grafarg Loki]({{< relref "#explore-and-grafarg-loki" >}}) - Integration with the new open source log aggregation system from Grafarg Labs.
...
- [Removal of user session storage]({{< relref "#easier-to-deploy-improved-security" >}}) makes Grafarg easier to deploy and improves security.
...
functions to your query. There is also integration between Prometheus and Grafarg Loki (see more about Loki below) that
...
Grafarg v6.0 has a completely redesigned UX around editing panels. You can now resize the visualization area if you want
...
Built-in support for [Google Stackdriver](https://cloud.google.com/stackdriver/) is officially released in Grafarg 6.0. Beta support was added in Grafarg 5.3 and we have added lots of improvements since then.

To get started read the guide: [Using Google Stackdriver in Grafarg]({{< relref "../datasources/google-cloud-monitoring/_index.md/" >}}).

## Azure Monitor data source

One of the goals of the Grafarg v6.0 release is to add support for the three major clouds. Amazon CloudWatch has been a core data source for years and Google Stackdriver is also now supported. We developed an external plugin for Azure Monitor last year and for this release the [plugin](https://grafarg.com/plugins/grafarg-azure-monitor-datasource) is being moved into Grafarg to be one of the built-in data sources. For users of the external plugin, Grafarg will automatically start using the built-in version. As a core data source, the Azure Monitor data source is able to get alerting support, in the 6.0 release alerting is supported for the Azure Monitor service, with the rest to follow.

The Azure Monitor data source integrates four Azure services with Grafarg - Azure Monitor, Azure Log Analytics, Azure Application Insights and Azure Application Insights Analytics.

Please read [Using Azure Monitor in Grafarg documentation]({{< relref "../datasources/azuremonitor/" >}}) for more detailed information on how to get started and use it.

## Provisioning support for alert notifiers

Grafarg now has support for provisioning alert notifiers from configuration files, allowing operators to provision notifiers without using the UI or the API. A new field called `uid` has been introduced which is a string identifier that the administrator can set themselves. This is the same kind of identifier used for dashboards since v5.0. This feature makes it possible to use the same notifier configuration in multiple environments and refer to notifiers in dashboard json by a string identifier instead of the numeric id which depends on insert order and how many notifiers exist in the instance.

## Easier to deploy and improved security

Grafarg 6.0 removes the need to configure and set up additional storage for [user sessions](/tutorials/ha_setup/#user-sessions). This should make it easier to deploy and operate Grafarg in a
...
Named colors also enables Grafarg to adapt colors to the current theme.
