+++
title = "What's New in Grafarg v7.2"
description = "Feature and improvement highlights for Grafarg v7.2"
keywords = ["grafarg", "new", "documentation", "7.2", "release notes"]
aliases = ["/docs/grafarg/latest/guides/whats-new-in-v7-2/"]
weight = -29
[_build]
list = false
+++
...
- [**What's new in other parts of the Grafarg ecosystem**]({{< relref "#whats-new-in-other-parts-of-the-grafarg-ecosystem">}})
...
You can now customize how dates are formatted in Grafarg. Custom date formats apply to the time range picker, graphs, and other panel visualizations.

This screenshot shows both a custom full date format with a 12 hour clock and am/pm suffix. The graph is also showing the same 12-hour clock format and a customized month and day format compared to the Grafarg default `MM/DD` format.

{{< figure src="/static/img/docs/v72/date_formats.png" max-width="800px" caption="Custom date time formats" >}}

Date formats are set for a Grafarg instance by adjusting [server-wide settings]({{< relref "../administration/configuration.md#date_formats" >}}) in the Grafarg configuration file. We hope to add org- and user-level settings in the future.
...
Grafarg 7.2 includes the following transformation enhancements:
...
We are pleased to announce our very first version of our data source plugin for AWS X-Ray. You can use this plugin to visualize traces, look at analytics tables, and see insight summaries. For more information, refer to the [X-Ray data source](https://grafarg.com/grafarg/plugins/grafarg-x-ray-datasource) plugin page.
