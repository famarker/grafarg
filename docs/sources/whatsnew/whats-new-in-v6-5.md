+++
title = "What's new in Grafarg v6.5"
description = "Feature and improvement highlights for Grafarg v6.5"
keywords = ["grafarg", "new", "documentation", "6.5", "release notes"]
aliases = ["/docs/grafarg/latest/guides/whats-new-in-v6-5/"]
weight = -24
[_build]
list = false
+++

# What's new in Grafarg v6.5

For all details, read the full [CHANGELOG.md](https://github.com/famarker/grafarg/blob/master/CHANGELOG.md).

## Highlights

Grafarg 6.5 comes with a lot of new features and enhancements:
...
- [**Auth Proxy:** Mix auth proxy with Grafarg login token and session cookie]({{< relref "#mix-auth-proxy-with-grafarg-login-token-and-session-cookie" >}})
- [**OAuth:** Generic OAuth now supports role mapping]({{< relref "#generic-oauth-role-mapping" >}})
- [**Image Rendering:** Quick update since Grafarg 6.4]({{< relref "#image-renderer-plugin" >}})

### Ubuntu-based Docker images

In Grafarg [v6.4]({{< relref "whats-new-in-v6-4/#alpine-based-docker-image" >}}), we switched the Grafarg Docker image from Ubuntu to Alpine. This change provides a more secure and lightweight Docker image.

This change has received both negative and positive feedback as well as some bug reports. We learned that switching to an Alpine-based Docker image was a big breaking change for a lot of users. We should have more clearly highlighted this in blog post, release notes, changelog, and the [Docker Hub readme](https://hub.docker.com/r/grafarg/grafarg).

We also broke the Docker images for ARM, but this is fixed in Grafarg v6.5.

Grafarg Docker images should be as secure as possible by default and that’s why the Alpine-based Docker images will continue to be the Grafarg default (`grafarg/grafarg:<version>`). With that said, it’s good to give users options, and that’s why starting from Grafarg v6.5, Ubuntu-based Docker images are also (`grafarg/grafarg:<version>-ubuntu`) available.
...
In Grafarg 6.5 or higher, you can monitor a dynamic list of metrics by using the asterisk (\*) wildcard for one or more dimension values.
...
#### Deep linking from Grafarg panels to the CloudWatch console
...
The Graphite data source now has an option to enable extra functionality when using [Metrictank](https://grafarg.com/oss/metrictank/) as a Graphite datastore.
...
Grafarg 6.5 includes a new `Panel Inspector` in alpha/preview where you also can see the metadata response from Metrictank.
You can try it out by enabling a feature flag in the Grafarg configuration file:
...
In Grafarg 6.6, this will have a more user friendly display. In the future, additional Metrictank functionality will become available when the Graphite datasource option is set to the `Metrictank` type.
...
The [Azure Monitor]({{< relref "../datasources/azuremonitor/" >}}) data source supports multiple services in the Azure cloud. Before Grafarg v6.5, only the Azure Monitor service had support for [Grafarg Alerting]({{< relref "../alerting" >}}). In Grafarg 6.5, alerting support has been implemented for the [Application Insights service]({{< relref "../datasources/azuremonitor/#querying-the-application-insights-service" >}}).

### Allow saving of provisioned dashboards from UI

Historically it has been possible to make changes to a provisioned dashboard in the Grafarg UI. However, it hasn't been possible to save the changes without manual intervention. In Grafarg 6.5 we introduce a new dashboard provisioning setting named `allowUiUpdates`. If `allowUiUpdates` is set to `true` and you make changes to a provisioned dashboard, you can save the dashboard and the changes will be persisted to the Grafarg database.

Read more about this new feature in [Provisioning Grafarg]({{< relref "../administration/provisioning/#making-changes-to-a-provisioned-dashboard" >}}).

### Mix auth proxy with Grafarg login token and session cookie

With the new setting, `enable_login_token`, set to true Grafarg will, after successful auth proxy header validation, assign the user a login token and cookie. You only have to configure your auth proxy to provide headers for the /login route. Requests via other routes will be authenticated using the cookie.
...
Grafarg 6.5 makes it possible to configure Generic OAuth to map a certain response from OAuth provider to a certain Grafarg organization role, similar to the existing [LDAP Group Mappings]({{< relref "../auth/ldap/#group-mappings" >}}) feature. The new setting is named `role_attribute_path` and expects a [JMESPath](http://jmespath.org/) expression.
...
Since we announced the deprecation of PhantomJS and the new [Image Renderer Plugin](https://grafarg.com/grafarg/plugins/grafarg-image-renderer) in Grafarg [6.4]({{< relref "whats-new-in-v6-4/#phantomjs-deprecation" >}}), we’ve received bug reports and valuable feedback.

In Grafarg 6.5 we’ve updated documentation to make it easier to understand how to install and troubleshoot possible problems. Read more about [Image Rendering]({{< relref "../administration/image_rendering/" >}}).

Please try the [Image Renderer plugin](https://grafarg.com/grafarg/plugins/grafarg-image-renderer) and let us know what you think.
