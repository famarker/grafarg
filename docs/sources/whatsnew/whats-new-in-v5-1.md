+++
title = "What's new in Grafarg v5.1"
description = "Feature and improvement highlights for Grafarg v5.1"
keywords = ["grafarg", "new", "documentation", "5.1", "release notes"]
aliases = ["/docs/grafarg/latest/guides/whats-new-in-v5-1/"]
weight = -15
[_build]
list = false
+++

# What's new in Grafarg v5.1

Grafarg v5.1 brings new features, many enhancements and bug fixes. This article will detail the major new features and enhancements.
...
In Grafarg v5.0 we introduced a new scrollbar component. Unfortunately this introduced a lot of issues and in some scenarios removed
the native scrolling functionality. Grafarg v5.1 ships with a native scrollbar for all pages together with a scrollbar component for
...
make the Grafarg user experience much better!

## Improved Docker image (breaking change)

Grafarg v5.1 brings an improved official docker image which should make it easier to run and use the Grafarg docker image and at the same time give more control to the user how to use/run it.

We've switched the id of the grafarg user running Grafarg inside a docker container. Unfortunately this means that files created prior to 5.1 won't have the correct permissions for later versions and thereby this introduces a breaking change.
We made this change so that it would be easier for you to control what user Grafarg is executed as (see examples below).
...
Grafarg v5.1 now ships with a built-in Microsoft SQL Server (MSSQL) data source plugin that allows you to query and visualize data from any
...
that data and define alert rules on it like with any of Grafarg's other core data sources.

Please read [Using Microsoft SQL Server in Grafarg documentation]({{< relref "../datasources/mssql/" >}}) for more detailed information on how to get started and use it.
...
In the table below you can see some examples and you can find all different options in the [Variables documentation](http://docs.grafarg.org/variables/templates-and-variables/#advanced-formatting-options).
...
Grafarg v5.1 brings an improved workflow for provisioned dashboards:
