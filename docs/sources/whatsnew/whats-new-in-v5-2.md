+++
title = "What's new in Grafarg v5.2"
description = "Feature and improvement highlights for Grafarg v5.2"
keywords = ["grafarg", "new", "documentation", "5.2", "release notes"]
aliases = ["/docs/grafarg/latest/guides/whats-new-in-v5-2/"]
weight = -16
[_build]
list = false
+++

# What's new in Grafarg v5.2

Grafarg v5.2 brings new features, many enhancements and bug fixes. This article will detail the major new features and enhancements.

- [Elasticsearch alerting]({{< relref "#elasticsearch-alerting" >}}) it's finally here!
- [Native builds for ARM]({{< relref "#native-builds-for-arm" >}}) native builds of Grafarg for many more platforms!
- [Improved Docker image]({{< relref "#improved-docker-image" >}}) with support for docker secrets
- [Security]({{< relref "#security" >}}) make your Grafarg instance more secure
...
Grafarg v5.2 ships with an updated Elasticsearch data source with support for alerting. Alerting support for Elasticsearch has been one of
...
Grafarg v5.2 brings an improved build pipeline with cross-platform support. This enables native builds of Grafarg for ARMv7 (x32) and ARM64 (x64).
...
The Grafarg docker image adds support for Docker secrets which enables you to supply Grafarg with configuration through files. More
...
Starting from Grafarg v5.2, when you login with the administrator account using the default password you'll be presented with a form to change the password.
We hope this encourages users to follow Grafarg's best practices and change the default administrator password.
...
By popular demand Grafarg now includes support for an alert notification channel type for [Discord](https://discordapp.com/).
...
Starting from Grafarg v5.2, a modified time range or variable are no longer saved by default. To save a modified
...
Grafarg v5.2 adds support for specifying an existing folder or creating a new one when importing a dashboard - a long-awaited feature since
Grafarg v5.0 introduced support for dashboard folders and permissions. The import dashboard page has also got some general improvements
