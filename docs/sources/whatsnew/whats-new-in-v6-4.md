+++
title = "What's new in Grafarg v6.4"
description = "Feature and improvement highlights for Grafarg v6.4"
keywords = ["grafarg", "new", "documentation", "6.4", "release notes"]
aliases = ["/docs/grafarg/latest/guides/whats-new-in-v6-4/"]
weight = -23
[_build]
list = false
+++
...
Grafarg 6.4 comes with a lot of new features and enhancements backed with tons of work around the data models and query execution that is going to enable powerful future capabilities.
...
- [**Plugins:** Alpha version of grafarg-toolkit]({{< relref "#alpha-version-of-grafarg-toolkit" >}})
...
With Grafarg 6.3 we introduced a new way of creating [Data Links](https://grafarg.com/blog/2019/08/27/new-in-grafarg-6.3-easy-to-use-data-links/).
Grafarg 6.4 improves Data Links and adds them to the Gauge and Bar Gauge and panels.
...
For more information about Data Links, refer to [data link](https://grafarg.com/docs/features/panels/graph/#data-link)
...
Grafarg 6.4 continues the work started in 6.3 of creating a data model and query execution lifecycle that can support robust analytics and streaming.  These changes are mostly structural and lay the foundation for powerful features in future releases.
...
[PhantomJS](https://phantomjs.org/), which is used for rendering images of dashboards and panels, have been deprecated and will be removed in a future Grafarg release. A deprecation warning will from now on be logged when Grafarg starts up if PhantomJS is in use.
...
Grafarg’s Docker image is now based on Alpine 3.10 and should from now on report zero vulnerabilities when scanning the image for security vulnerabilities.
...
The feature is currently limited to Grafarg Server Admins.
...
A common request from Enterprise users have been to be able to set up reporting for Grafarg, and now it’s here. A report is simply a PDF of a Grafarg dashboard, outside of just generating a PDF you can set up a schedule so that you can get the report emailed to yourself (or whoever is interested) whenever it suits you.
...
GitLab OAuth gets support for Team Sync, making it possible to synchronize your GitLab Groups with Teams in Grafarg.

[Read more about Team Sync](https://grafarg.com/docs/auth/team-sync/).
