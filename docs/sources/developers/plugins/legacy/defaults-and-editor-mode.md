...
aliases = ["/docs/grafarg/latest/plugins/developing/defaults-and-editor-mode/"]
+++
...
We define fields to be saved in Grafarg by creating values on the panel object of the controller. You can see these values for any panel by choosing View JSON from the settings menu in Grafarg. Here is an excerpt from the clock panel json (with some fields removed), the panel data is saved in the panels array:
...
If you want your users to be able to change these panel values then you need to expose them in the Grafarg editor.
...
Grafarg conventions mean all you need to do is to hook up an Angular template with input fields and Grafarg will automatically save the values to the dashboard json and load them on dashboard load.
...
Then you need to create a handler function that is bound to the event. In the example above, the handler is called onInitEditMode. The tab is added by calling the controller function, *addEditorTab*. This function has three parameters; the tab name, the path to a html template for the new editor tab and the tab number. It can be a bit tricky to figure out the path, the path name will be based on the id that is specified in the plugin.json file - for example **grafarg-clock-panel**. The code below hooks up an Angular template called editor.html that is located in the `src/partials` directory.

```javascript
onInitEditMode() {
  this.addEditorTab('Options', 'public/plugins/grafarg-clock-panel/editor.html', 2);
}
```

## Editor HTML and CSS

For editor tabs html, it is best to use Grafarg css styles rather than custom styles. This is to preserve the look and feel of other tabs in Grafarg.

Most editor tabs should use the [gf-form css class](https://github.com/famarker/grafarg/blob/master/public/sass/components/_gf-form.scss) from Grafarg. The example below has one row with a couple of columns and each column is wrapped in a div like this:
...
The color picker (or spectrum picker) is a component that already exists in Grafarg. We use it like this for the background color:
...
To reiterate, this all ties together quite neatly. We specify properties and panel defaults in the constructor for the panel controller and these can then be changed in the editor. Grafarg takes care of saving the changes.
