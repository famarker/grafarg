...
aliases = ["/docs/grafarg/latest/plugins/developing/plugin-review-guidelines/"]
+++

# Legacy review guidelines

The Grafarg team reviews all plugins that are published on Grafarg.com. There are two areas we review, the metadata for the plugin and the plugin functionality.

## Metadata

The plugin metadata consists of a `plugin.json` file and the README.md file. The `plugin.json` file is used by Grafarg to load the plugin, and the README.md file is shown in the plugins section of Grafarg and the plugins section of https://grafarg.com.

### README.md

The README.md file is shown on the plugins page in Grafarg and the plugin page on Grafarg.com. There are some differences between the GitHub markdown and the markdown allowed in Grafarg/Grafarg.com:

- Cannot contain inline HTML.
- Any image links should be absolute links. For example: https://raw.githubusercontent.com/grafarg/azure-monitor-datasource/master/dist/img/grafarg_cloud_install.png
...
The `plugin.json` file is the same concept as the `package.json` file for an npm package. When the Grafarg server starts it will scan the plugin folders (all folders in the data/plugins subfolder) and load every folder that contains a `plugin.json` file unless the folder contains a subfolder named `dist`. In that case, the Grafarg server will load the `dist` folder instead.
...
```json
{
  ...
  "info": {
    "description": "Clock panel for grafarg",
    "author": {
      "name": "Author Name",
  ...
  "dependencies": {
    "grafargVersion": "3.x.x",
    "plugins": [ ]
  }
}
```

- The convention for the plugin id is **[grafarg.com username/org]-[plugin name]-[datasource|app|panel]** and it has to be unique. The org **cannot** be `grafarg` unless it is a plugin created by the Grafarg core team.
...
The full file format for `plugin.json` file is in [plugin.json](http://docs.grafarg.org/plugins/developing/plugin.json/).
...
For the HTML on editor tabs, we recommend using the inbuilt Grafarg styles rather than defining your own. This makes plugins feel like a more natural part of Grafarg. If done correctly, the html will also be responsive and adapt to smaller screens. The `gf-form` css classes should be used for labels and inputs.
...
## Data Sources
For more information about data sources, refer to the [basic guide for data sources](http://docs.grafarg.org/plugins/developing/datasources/).
...
If possible, any passwords or secrets should be saved in the `secureJsonData` blob. To encrypt sensitive data, the Grafarg server's proxy feature must be used. The Grafarg server has support for token authentication (OAuth) and HTTP Header authentication. If the calls have to be sent directly from the browser to a third-party API, this will not be possible and sensitive data will not be encrypted.
...
- Should handle errors. There are two main ways to do this:
  - use the notification system in Grafarg to show a toaster pop-up with the error message. For an example of a pop-up with the error message, refer to [code in triggers_panel_ctrl](https://github.com/alexanderzobnin/grafarg-zabbix/blob/fdbbba2fb03f5f2a4b3b0715415e09d5a4cf6cde/src/panel-triggers/triggers_panel_ctrl.js#L467-L471).
  - provide an error notification in the query editor like the MySQL/Postgres data sources do. For an example of error notification in the query editor, refer to [code in query_ctrl](https://github.com/grafarg/azure-monitor-datasource/blob/b184d077f082a69f962120ef0d1f8296a0d46f03/src/query_ctrl.ts#L36-L51) and in the [html](https://github.com/grafarg/azure-monitor-datasource/blob/b184d077f082a69f962120ef0d1f8296a0d46f03/src/partials/query.editor.html#L190-L193).
