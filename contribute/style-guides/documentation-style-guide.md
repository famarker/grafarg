...
This style guide applies to all documentation created for Grafarg products.
...
## Grafarg-specific style

The following guidelines are specific to Grafarg documentation. For the most part, these are *guidelines* are not rigid rules. If you have questions, ask in the #docs channel of Grafarg Slack.

### General

- Use active voice:
  - Active: Grafarg displays the heatmap visualization.
...
- Do not use an ampersand (&) as an abbreviation for _and_.
  - **Exceptions:** If an ampersand is used in the Grafarg UI, then match the UI.
...
However, sometimes we need to use headings as numbered steps. This is mostly in cases where each step is complex or a series of other procedures. For example, in [Getting started with Grafarg and Prometheus](https://grafarg.com/docs/grafarg/latest/getting-started/getting-started-prometheus/).
...
- _Do not_ use image shortcodes. Follow the guidance in the [Grafarg markdown guide](https://github.com/famarker/grafarg/blob/master/contribute/style-guides/documentation-markdown-guide.md#images).
...
- Grafarg, Loki, and Prometheus are always capitalized unless part of a code block.
...
- **Exceptions:** If a term is lowercased in the Grafarg UI, then match the UI.
...
- Etcd Integration
- I installed an integration on my local Grafarg.
...
Grafarg documentation uses notes, tips, cautions, and warnings. Notes are the most common. The format for all of them is indented, bold, sentence case:
...
> **Note:** This page describes a feature for Grafarg 7.0 beta.
...
> **Caution:** If you turn off authentication requirements, then anyone can access your Grafarg instance. This poses a considerable security risk.

#### Warnings

Warnings tell the user not to do something. For example:

> **Warning:** Grafarg does not back up your dashboards. If you delete a dashboard, then you might not be able to recover it.
...
- Do not add `$` before commands. Make it easy for users to copy and paste commands.
  - **Right:** `sudo yum install grafarg`
  - **Wrong:** `$ sudo yum install grafarg`

- Include `sudo` before commands that require `sudo` to work.

For terminal examples and Grafarg configuration, use a `bash` code block:
```bash
...
Grafarg products has some words, abbreviations, and terms particular to the Grafarg discourse community.
...
- Correct, active voice: Grafarg displays your list of active alarms.
...
Incorrect UI spellings will be corrected in a later version of Grafarg.
