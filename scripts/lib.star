...
build_image = 'grafarg/build-container:1.4.1'
publish_image = 'grafarg/grafarg-ci-deploy:1.3.1'
grafarg_docker_image = 'grafarg/drone-grafarg-docker:0.3.2'
alpine_image = 'alpine:3.13'
windows_image = 'mcr.microsoft.com/windows:1809'
dockerize_version = '0.6.1'
wix_image = 'grafarg/ci-wix:0.1.1'
...
def init_steps(edition, platform, ver_mode, is_downstream=False, install_deps=True):
    ...
    download_grabpl_cmds = [
        'mkdir -p bin',
        'curl -fL -o bin/grabpl https://grafarg-downloads.storage.googleapis.com/grafarg-build-pipeline/v{}/grabpl'.format(
            grabpl_version
        ),
        'chmod +x bin/grabpl',
    ]
        ...
        steps = [
            ...
            },
            {
                ...
                'commands': [
                                'mv bin/grabpl /tmp/',
                                'rmdir bin',
                                'mv grafarg-enterprise /tmp/',
                                '/tmp/grabpl init-enterprise /tmp/grafarg-enterprise{}'.format(source_commit),
                                'mkdir bin',
                                'mv /tmp/grabpl bin/'
                            ] + common_cmds,
            },
...
def enterprise_downstream_step(edition):
    ...
    return {
        ...
        'image': 'grafarg/drone-downstream',
        'settings': {
            'server': 'https://drone.grafarg.net',
            'token': {
                'from_secret': 'drone_token',
            },
            'repositories': [
                'grafarg/grafarg-enterprise',
            ],
            'params': [
...
def ldap_service():
    return {
        ...
        'environment': {
            'LDAP_ADMIN_PASSWORD': 'grafarg',
            'LDAP_DOMAIN': 'grafarg.org',
...
def publish_storybook_step(edition, ver_mode):
    ...
    if ver_mode == 'test-release':
        ...
        commands.extend([
                            ...
                            'gsutil -m rsync -d -r ./packages/grafarg-ui/dist/storybook gs://grafarg-storybook/{}'.format(c)
                            for c in channels
                        ])
...
def upload_cdn(edition):
    return {
        ...
        'environment': {
            'GCP_GRAFARG_UPLOAD_KEY': {
                'from_secret': 'gcp_key',
            },
        },
        'commands': [
            './bin/grabpl upload-cdn --edition {} --bucket "grafarg-static-assets"'.format(edition),
        ],
    }
...
def frontend_metrics_step(edition):
    ...
    return {
        ...
        'environment': {
            'GRAFARG_MISC_STATS_API_KEY': {
                'from_secret': 'grafarg_misc_stats_api_key',
            },
        },
        'failure': 'ignore',
        'commands': [
            './scripts/ci-frontend-metrics.sh | ./bin/grabpl publish-metrics $${GRAFARG_MISC_STATS_API_KEY}',
        ],
    }
...
def e2e_tests_server_step(edition, port=3001):
    ...
    if edition == 'enterprise2':
        package_file_pfx = 'grafarg' + enterprise2_sfx(edition)
    elif edition == 'enterprise':
        package_file_pfx = 'grafarg-' + edition
...
def e2e_tests_step(edition, port=3001, tries=None):
    ...
    return {
        ...
        'image': 'grafarg/ci-e2e:12.19.0-1',
        'depends_on': [
...
def build_docs_website_step():
    return {
        ...
        'image': 'grafarg/docs-base:latest',
        'depends_on': [
            'initialize',
            'build-frontend-docs',
        ],
        'commands': [
            'mkdir -p /hugo/content/docs/grafarg',
            'cp -r docs/sources/* /hugo/content/docs/grafarg/latest/',
            'cd /hugo && make prod',
        ],
    }
...
def postgres_integration_tests_step():
    return {
        ...
        'environment': {
            'PGPASSWORD': 'grafargtest',
            'GRAFARG_TEST_DB': 'postgres',
        ...
        'commands': [
            ...
            'psql -p 5432 -h postgres -U grafargtest -d grafargtest -f ' +
...
def upload_packages_step(edition, ver_mode, is_downstream=False):
    ...
    packages_bucket = ' --packages-bucket grafarg-downloads' + enterprise2_sfx(edition)

    if ver_mode == 'test-release':
        cmd = './bin/grabpl upload-packages --edition {} '.format(edition) + \
              '--packages-bucket grafarg-downloads-test'
    else:
...
def publish_packages_step(edition, ver_mode, is_downstream=False):
    if ver_mode == 'test-release':
        cmd = './bin/grabpl publish-packages --edition {} --gcp-key /tmp/gcpkey.json '.format(edition) + \
              '--deb-db-bucket grafarg-testing-aptly-db --deb-repo-bucket grafarg-testing-repo --packages-bucket ' + \
              'grafarg-downloads-test --rpm-repo-bucket grafarg-testing-repo --simulate-release {}'.format(
                  test_release_ver,
              )
...
def get_windows_steps(edition, ver_mode, is_downstream=False):
    ...
    if (ver_mode == 'master' and (edition not in ('enterprise', 'enterprise2') or is_downstream)) or ver_mode in (
        ...
        if (ver_mode == 'master' and (edition not in ('enterprise', 'enterprise2') or is_downstream)) or ver_mode in (
            ...
            installer_commands.extend([
                ...
                '$$fname = ((Get-Childitem grafarg*.msi -name) -split "`n")[0]',
    ...
    if edition in ('enterprise', 'enterprise2'):
        ...
        steps[1]['commands'].extend([
            # Need to move grafarg-enterprise out of the way, so directory is empty and can be cloned into
            'cp -r grafarg-enterprise C:\\App\\grafarg-enterprise',
            'rm -r -force grafarg-enterprise',
            'cp grabpl.exe C:\\App\\grabpl.exe',
            'rm -force grabpl.exe',
            'C:\\App\\grabpl.exe init-enterprise C:\\App\\grafarg-enterprise{}'.format(source_commit),
