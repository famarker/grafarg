...
const changelogTaskRunner = ({ milestone }: ChangelogOptions) =>
  useSpinner('Generating changelog', async () => {
    ...
    const toolkitIssues = issues.filter((item: any) =>
      item.labels.find((label: any) => label.name === 'area/grafarg/toolkit')
    );
    const grafargUiIssues = issues.filter((item: any) =>
      item.labels.find((label: any) => label.name === 'area/grafarg/ui')
    );

    let markdown = '';

    markdown += getPackageChangelog('Grafarg', issues);
    markdown += getPackageChangelog('grafarg-toolkit', toolkitIssues);
    markdown += getPackageChangelog('grafarg-ui', grafargUiIssues);

    console.log(markdown);
  });

function getMarkdownLineForIssue(item: any) {
  const githubGrafargUrl = 'https://github.com/famarker/grafarg';
  let markdown = '';
  ...
  if (!item.pull_request) {
    markdown += '* ' + title + '.';
    markdown += ` [#${item.number}](${githubGrafargUrl}/issues/${item.number})`;
  } else {
    markdown += '* ' + title + '.';
    markdown += ` [#${item.number}](${githubGrafargUrl}/pull/${item.number})`;
